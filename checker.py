...
def check_proxies():
    ...
    total = len(proxies)
    output_text.insert(tk.END, f"🔍 開始查詢共 {total} 筆 Proxy...\n\n")
    output_text.update()
    ...
    for index, raw in enumerate(proxies, start=1):
        ...
        try:
            ...
            result_line = f"{progress_text}🔹 Proxy: {raw}\n    → IP: {ip}\n\n"
            output_text.insert(tk.END, result_line, "success")
            queried_results.append((raw, ip))
        except Exception as e:
            error_line = f"{progress_text}❌ Proxy: {raw}\n    → 錯誤：{e}\n\n"
...
# 建立 GUI 視窗
window = tk.Tk()
window.title("SOCKS5 Proxy IP 查詢工具")
